...
class KubernetesQueryAgent:
    ...
    def collect_comprehensive_information(self):
        ...
        try:
            ...
            all_pods = self.core_v1.list_pod_for_all_namespaces()
            cluster_info['total_pod_count'] = len(all_pods.items)
            cluster_info['running_pod_count'] = sum(
                1 for pod in all_pods.items if pod.status.phase == 'Running'
            )
            cluster_info['pods'] = [
                {
                    'name': pod.metadata.name,
            ...
            # Preprocess Readiness Probes
            cluster_info['readiness_probes'] = {
                pod.metadata.name: [
            ...
            cluster_info['env_variables'] = {
                pod.metadata.name: {
                    container.name: {
                        env.name: env.value
                        for env in container.env
                    } for container in pod.spec.containers if container.env
                } for pod in all_pods.items
            }
            ...
            for namespace in [ns['name'] for ns in cluster_info['namespaces']]:
                try:
                    ...
                    cluster_info['service_to_namespace'].update({
    ...
    def get_running_pod_count(self):
        return self.cluster_context['running_pod_count']

    def get_readiness_probe(self, pod_name):
        return self.cluster_context['readiness_probes'].get(pod_name, ["Not Found"])

    def get_env_variable(self, pod_name, container_name, env_var_name):
        containers = self.cluster_context['env_variables'].get(pod_name, {})
        container_envs = containers.get(container_name, {})
        return container_envs.get(env_var_name, "Not Found")

    def get_service_namespace(self, service_name):
        return self.cluster_context['service_to_namespace'].get(service_name, "Not Found")
    ...
    def query_openai(self, query):
        """
        Send query to OpenAI with cluster context or resolve using local data
        """
        try:
            # Attempt to resolve query using preprocessed data
            if "how many pods are in the cluster" in query.lower():
                return str(self.get_running_pod_count())
            if "readiness probe" in query.lower():
                pod_name = query.split()[-1]
                return self.get_readiness_probe(pod_name)[0]
            if "namespace" in query.lower() and "service" in query.lower():
                service_name = query.split()[-1]
                return self.get_service_namespace(service_name)
            if "environment variable" in query.lower():
                parts = query.split()
                pod_name, container_name, env_var_name = parts[-3], parts[-2], parts[-1]
                return self.get_env_variable(pod_name, container_name, env_var_name)

            # Query OpenAI if not resolved locally
            response = self.openai_client.chat.completions.create(
                model="gpt-4",
                messages=[
                    {
                        ...
                        "content": """
                            You are a Kubernetes cluster information assistant. Follow these rules strictly:
                            1. Return direct answers only.
                            2. If no data is found, return "Not Found".
                        """
                    },
                    {
                        "role": "user", 
